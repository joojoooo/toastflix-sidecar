...
class OffsetStore:
    ...
    # Fields produced by SyncEngine at the reference commit (4128f644).
    # Dashboard state and local edit history must never enter the VPS offset.
    REMOTE_OFFSET_FIELDS = (
        "status", "offset", "rate", "confidence", "deviation", "sync_mode",
        "video_duration", "audio_duration", "measurements", "video_start_time",
        "sync_algorithm", "cache_key", "cached", "drift_per_hour",
        "candidate_rate", "end_deviation", "linear_deviation",
    )
    ...
    def _remote_offset_result(cls, value):
        """Send only the offset fields generated by the original sync engine."""
        result = {name: value[name] for name in cls.REMOTE_OFFSET_FIELDS if name in value}
        # The reference engine added `cached` only for a cache hit. The fork
        # adds `cached: false` to newly calculated results for its dashboard.
